        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('home.projects.update', $project) }}" method="POST">
                            @method('put')
                            @csrf
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="description" id="inputDescription" class="form-control" rows="10" style="width: 100%">{{ old('description', $project->description) }}</textarea>
                                @error('description')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="user_id" id="inputWorker" class="form-control">
                                    @foreach ($workers as $worker)
                                        <option value="{{ $worker->id }}" {{ old('user_id', $project->user_id) == $worker->id ? 'selected' : '' }}>{{ $worker->name }}</option>
                                    @endforeach
                                </select>
                                @error('user_id')
                                    <span class="text-danger">{{ $message }}</span>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="status" id="inputStatus" class="form-control">
                                    @foreach (App\Models\Project::STATUS as $status)
                                        <option value="{{ $status }}"
                                            {{ old('status', $project->status) == $status ? 'selected' : '' }}>{{ ucfirst($status) }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('status')
