        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('home.tasks.update', $task) }}" method="POST">
                            @method('put')
                            @csrf
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="description" id="inputDescription" class="form-control" rows="10" style="width: 100%">{{ old('description', $task->description) }}</textarea>
                                @error('description')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="status" id="inputStatus" class="form-control">
                                    @foreach (App\Models\Project::STATUS as $status)
                                        <option value="{{ $status }}"
                                            {{ old('status', $task->status) == $status ? 'selected' : '' }}>{{ ucfirst($status) }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('status')
